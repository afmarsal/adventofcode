...
def part1(filename, path):
    ...
    dir = RIGHT
    pos = (0, starting_x)
    log(f'Starting: {pos}, direction: {dir}')
    idx = 0
    reading_steps = True
    while idx < len(path):
        # Parse path
        ...
        idx += 1
        log(f'idx: {idx}, rotate: {rotate}, steps: {steps}, pos: {pos[0]}, {pos[1]}')
        ...
        match rotate:
            ...
            case 'L':
                ...
                log(f'New direction: {dir}')
            case _:
                log(f'Moving {steps} steps in direction {dir} from {pos[0]}, {pos[1]}')
                for s in range(steps):
                    ...
                    if lines[new_y][new_x] == '#':
                        log(f'Found wall at: {new_x}, {new_y}')
                        break
                    pos = new_y, new_x
                log(f'Moved to {pos[0]}, {pos[1]}')

    return (pos[0] + 1) * 1000 + (pos[1] + 1) * 4 + DIRECTIONS.index(dir)
...
class TestPart2(unittest.TestCase):
    def test_sample(self):
        self.assertEqual(-2, part2('sample.txt', smp))
